...
function SiaeTree(options)
{
	...
	this.treejs.on('select_node.jstree', function(node, selected, event)
	{
		...
		if( sno.rome )
		{
			...
			$(self).trigger( 'cartosiae.nodes_selected', [[sno]] );
		}
		else
		{
			...
			$(self).trigger( 'cartosiae.nodes_selected', [nodes] );
		}
	});
	...
	this.treejs.on('deselect_node.jstree', function(node, deselected, event){
		...
		if( sno.rome )
			$(self).trigger( 'cartosiae.nodes_deselected', [[sno]] );
		else
		{
			var nodes = [];
			deselected.node.children_d.forEach(function(n){
				n = self.treejs.jstree('get_node', n);
				// process only item with "rome" attribute -> leaf node
				if( n.original.rome )
					nodes.push( n.original );
			});
			$(self).trigger( 'cartosiae.nodes_deselected', [nodes] );
		}
	});
	...
	$('#'+options.treeSelectAllId).on('click',function(e)
	{
		...
		nodes.forEach(function(n){
			...
			if( n.original.rome )
				nodesOrigin.push( n.original );
		});
		$(self).trigger( 'cartosiae.nodes_selected', [nodesOrigin] );
	});
	...
	$('#'+options.treeSelectNoneId).on('click',function(e)
	{
		...
		$(self).trigger( 'cartosiae.nodes_all_deselected' );
