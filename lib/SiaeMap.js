...
function SiaeMap( options )
{
	...
	this.markers.on('clustermouseover', function (e)
	{
		...
		e.layer.getAllChildMarkers().some(function(m,i)
		{
			if( i >= 3 )
			{
				content += ' ...';
			...
			if( ! contentEmpty ) // faster than (content.length>0)
				content += ', ';
			content += m.options.title ;
			contentEmpty = false ;
		});
		// Use of Leaflet plugin Leaflet.label
	...
	this.markers.on('clustermouseout', function (e)
	{
		// Do not do that, it doesn't work
		// https://github.com/Leaflet/Leaflet.markercluster/issues/495#issuecomment-145663552
		//e.layer.unbindLabel();
	});
